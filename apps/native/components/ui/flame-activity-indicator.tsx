...
export function FlameActivityIndicator({
	...
	const defaultIconColor = getFlameIconDefaultColor(colors);
	const progress = useFlamePulseAnimation(animating);
	const {
		outer: flameColor,
		inner: coreColor,
		glow: resolvedGlowTint,
	} = resolveFlameColors(
		colors,
		color,
		outerTint,
	...
	return (
		...
		>
			...
			/>
			<Animated.View
				style={[styles.iconLayer, { transform: motion.outerTransform }]}
			>
				<FlameIcon color={flameColor} size={size} />
			</Animated.View>
			<Animated.View
				style={[styles.iconLayer, { transform: motion.innerTransform }]}
			>
				<FlameIcon color={coreColor} size={size * innerScale} />
			</Animated.View>
		</View>
	);
}
