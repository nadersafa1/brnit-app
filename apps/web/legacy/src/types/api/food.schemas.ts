...
const categoryIdsSchema = z
  ...
  .max(20, 'At most 20 categories allowed')
  .transform(ids => [...new Set(ids)])

const categoryIdsUpdateSchema = z
  .array(z.uuid('Invalid category ID'))
  .min(1, 'At least one category is required')
  .max(20, 'At most 20 categories allowed')
  .transform(ids => [...new Set(ids)])
...
export const createFoodItemSchema = z
  ...
  .refine(data => !unitRequiresGramsPerUnit(data.unit) || (data.gramsPerUnit != null && data.gramsPerUnit > 0), {
...
export const updateFoodItemSchema = z
  .object({
    ...
    categoryIds: categoryIdsUpdateSchema,
...
/** FormData parsing for POST create — categoryIds supplied by route from getAll */
...
export const foodItemAlternativesQuerySchema = z.object({
  quantity: z.coerce.number().positive('Quantity must be positive').max(10000, 'Quantity must be at most 10000'),
